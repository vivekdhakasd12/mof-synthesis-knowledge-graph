"""Render the report chapters to LaTeX and compile a hyperlinked PDF.

Why this exists alongside `build_report.py`: the HTML path produces a document that reads
well on screen and emails cleanly, but a submitted thesis wants a navigable PDF, a real
table of contents whose entries jump to their section, sidebar bookmarks, and numbered
figures that can be cross-referenced. LaTeX gives all of that.

**The markdown chapters stay the single source of truth.** Both builders read the same
files, so the HTML and the PDF cannot drift apart in content. Nothing here is authored by
...
Run:  python docs/report/build_latex.py
      python docs/report/build_latex.py --no-pdf     (write the .tex only)
"""
...
TEX = REPORT / "report_latex.tex"
PDF = REPORT / "report_latex.pdf"
